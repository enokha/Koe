
// HomePage.js
import React from 'react';
import { Navigate } from 'react-router-dom';
import SideBar from '../components/SideBar';
import AddTweet from '../components/AddTweet'; // Adjust the path accordingly
import GetTweets from '../components/GetTweets'; // Adjust the path accordingly
import Logout from '../components/Logout'; // Adjust the path accordingly
import Widgets from '../components/Widgets'; // Import the Widgets component
import '../styles/HomePage.css';

const HomePage = () => {
  // Check if the user is logged in
  const isLoggedIn = localStorage.getItem('loginState') === 'yes';

  if (!isLoggedIn) {
    return <Navigate to="/login" />;
  }

  return (
    <div className="homePage">
      {/* Sidebar */}
      <div className="sidebar">
        <SideBar />
        <Logout />
      </div>

      {/* Feed */}
      <div className="feed">
        <img
          alt="twitter logo"
          className="h-10"
          src="data:image/svg+xml;base64,"
        />
        <p className="m-5 text-2xl font-semibold text-center text-white">
          Welcome to Twitter
        </p>
        <AddTweet />
        <GetTweets />
      </div>

      {/* Widgets */}
      <div className="widgets">
        <Widgets />
      </div>
    </div>
  );
};

export default HomePage;
